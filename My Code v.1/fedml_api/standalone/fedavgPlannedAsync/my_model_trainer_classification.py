...
class MyModelTrainer(ModelTrainer):
    ...
    def train(self, train_data, device, round_number, args):
        model = self.model
        ...
        if args.client_optimizer == "sgd":
            ...
            if args.lr == 1000:
                ...
                lr_end = 0.01
                a = lr_start
                #print(f"args.comm_round = {args.comm_round}")
                b = (1 / args.comm_round) * np.log(lr_end / lr_start)
                #b = 0.99
                LR = a * math.e**(round_number * b)
                #print(f"LR = {LR}")
        ...
        for epoch in range(args.epochs):
            ...
            #    print(f"111train_data = {labels}, batch_idx = {batch_idx}")
            train_data = self.shuffle_data_each_Epoch(train_data)
            #for batch_idx, (x, labels) in enumerate(train_data):
            #    print(f"222train_data = {labels}, batch_idx = {batch_idx}")
            batch_loss = []
            for batch_idx, (x, labels) in enumerate(train_data):
                x, labels = x.to(device), labels.to(device)
                #if batch_idx == 0:
                #   print(f"train_data = {labels}, batch_idx = {batch_idx}")
                model.zero_grad()
                log_probs = model(x)
                #labels = labels.long()
        ...
        for epoch in range(args.epochs):
            batch_loss = []
            train_data = self.shuffle_data_each_Epoch(train_data)
            #model_old = copy.deepcopy(w_global_global)
            for batch_idx, (x, labels) in enumerate(train_data):
                x, labels = x.to(device), labels.to(device)
            ...
            epoch_loss.append(sum(batch_loss) / len(batch_loss))
            '''
            #print(f"gradients_all = {gradients_all}")
            #print(f"##### param.grad  = {dir(model)}")
            #print(dir(model))
            #print(f"%%%%% gradients_all = {gradients_all}")
            #gradientt = copy.deepcopy(w_global_global)
            #model_new = copy.deepcopy(model.state_dict())


            #print(f"!!!!!!!!before!!!!!!!!! = {gradientt}")


            #print(f"gradientt = {gradientt}")
            #print(gradients_all[0][list(gradientt.keys())[1]][0])
            #print(f"############################################")
            #print(gradients_all[1][list(gradientt.keys())[1]][0])
            #for param in model.parameters():
            #        gradientt[list(gradientt.keys())[counter]] = param.grad
            #        counter += 1
            #for num_batch in range(len(gradients_all)):
            # for param in gradients_all[num_batch].parameters():
            #        gradientt[list(gradientt.keys())[counter]] = gradients_all[num_batch].grad
            #        counter += 1
            #print(f"!!!!!!!!after!!!!!!!!! = {gradientt}")


            #for j in range(len(gradient.keys())):
            #        gradient[list(gradient.keys())[j]] ==  gradient_torch[j]
            #print(f" %%%%%%%%%%%%%%Gradients =  {gradient_torch}")
    ...
    def shuffle_data_each_Epoch(self,train_data):
      ...
      #print(f"%%%%train_data[0][0][1] = {train_data[0][0][1]}")
      ...
      #print(f"###train_data[0][0][1] = {batched_y[0][0][1]}")
      return train_data
