...
class FedAvgAPI(object):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                if self.AMP_SPAR[0] == False and self.AMP_SPAR[1] == True:
                    ...
                    count_non_zeroone_orbit = self.count_non_zero_elements(self.number_non_zero_entries[0:5])
                    print(f"&&& count_non_zeroone_orbit = {count_non_zeroone_orbit}")
                    count_non_zero_all = self.count_non_zero_elements(self.number_non_zero_entries)
                    print(f"&&& count_non_zero_all = {count_non_zero_all}")
            ...
            if self.AMP_SPAR[0] == True and self.AMP_SPAR[1] == False:
               ...
               for idx_orb in range(self.number_orbits):
                   size_data_SPAR_method = size_data_SPAR_method + self._size_data_spar(vectors_local_Spar[idx_orb*Num_sat_per_orbit: idx_orb*Num_sat_per_orbit + Num_sat_per_orbit]) # Each entry of
                   print(f" ### = {idx_orb*Num_sat_per_orbit, idx_orb*Num_sat_per_orbit + Num_sat_per_orbit}")
                   print(f"^^^size_data_SPAR_method = {size_data_SPAR_method}")
                   print(f"%%%%%len vectors_local_Spar[0] = {len(vectors_local_Spar[0])}")
                   #print(f"%%%^^^size_data_SPAR_method[0] = {size_data_SPAR_method[0]}")
                   #print(f"***^^^size_data_SPAR_method[0][0] = {size_data_SPAR_method[0][0]}")
               size_bit_AMP = self._size_data_AMP(vectors_local_AMP)
               ...
               for idx_orb in range(self.number_orbits):
                  ...
                  print(f"222%%%%%len vectors_local_Spar[0] = {len(vectors_local_AMP[0])}")
               Num_sat_per_orbit = int(len(self.client_list) / self.number_orbits)
               for idx_orb in range(self.number_orbits):
    ...
    def _size_data_spar(self, w_sparsification):
        ...
        x88 = torch.tensor([1], dtype=torch.float64)
        print(f"+++++++++x88.element_size() = {x88.element_size()}")
        print(f"333values  ={len(values[0])}")
        print(f"444indices  ={len(indices[0])}")
        ...
        print(f"99!!!!flattened_list = {len(flattened_list)}")

        flattened_list = torch.tensor(flattened_list, dtype=torch.float64)
        for i in range(len(flattened_list)):
                   #print(flattened_list[i].element_size() * flattened_list[i].nelement() * 8)
                   sizee_value = sizee_value + (flattened_list[i].element_size() * flattened_list[i].nelement() * 8)

        sizee = sizee_ind + sizee_value
        print(f" 77** = {sizee_ind}")
        print(f" 88&& = {sizee_value}")
        return sizee
    ...
    def _local_test_on_all_clients(self, round_idx, size_bit, size_data_spar_method):
        ...
        wandb.log({"Upload_Bits_each_Orbit_Spar": size_data_spar_method, "round": round_idx})
        logging.info(stats_upload_size)
