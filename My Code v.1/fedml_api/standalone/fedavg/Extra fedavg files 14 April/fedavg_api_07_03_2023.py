...
class FedAvgAPI(object):
   def __init__(self, dataset, device, args, model_trainer):
        ...
        self.p = 1
        self.AMP_TopK = [True, False] # % The first entry: AMP algorithm and the second entry: TopK -- > TopK = [False , True]  and AMP = [True, False]
        self.test_acc_Sync = []
   ...
   def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            if self.AMP_TopK[0] == True and self.AMP_TopK[1] == False:
               ...
               size_data_TopK_method = self._TopK_Alg_Size_MPs(vectors_local_Spar, Num_sat_per_orbit)
               size_data_AMP_method = self._AMP_Alg_Size_MPs(vectors_local_Spar, Num_sat_per_orbit)
   ...
   def _size_data_TopK(self,w_sparsification):
            ...
            number_sat_each_orbit  = len(x0)
            print(f"RRRR number_sat_each_orbit = {number_sat_each_orbit}")
            number_groups_each_orbit = math.floor(number_sat_each_orbit/2)+1    # grouping the satellite in each orbit based on their position to transmit the weights
            group_index  = [0 for i in range(number_groups_each_orbit)]
            indexes = torch.arange(number_sat_each_orbit)
            #print(indexes)
            ...
            print(f"TopK flattened_list[i].element_size() = {flattened_list_ind_tensor.nelement()}")
            #print(sizee_ind)
            ...
            #print(f"LLLL len(flattened_list_ind_tensor) : {len(flattened_list_ind_tensor)}")
            #print(f"$$$ flattened_list_ind_tensor.element_size() = {flattened_list_ind_tensor.element_size()}")
            #print(f"^^^ flattened_list_ind_tensor.nelement() = {flattened_list_ind_tensor.nelement()}")
