    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                w, gradients = client.train(w_global_temp_to_train, w_global_temp_to_train)
                print(f"gradients = {gradients}")
                #print(f" w = {w}, idx = {idx}")
                w_sparsification = copy.deepcopy(w)
                w_residual_user = copy.deepcopy(w_residual_all_users[idx])
                #print(f" w_residual_user = {w_residual_user}, w_sparsification = {w_sparsification}")
                #w_sparsification1, w_residual_new = self._digital_sparsification(w_sparsification, w_residual_user, idx)
                w_sparsification1, w_residual_new = self._Strom_Threshold_sparsification(w_sparsification, w_residual_user, idx)
            ...
            #print(f"w_locals = {w_locals}")
            #print(f"w_residual_all_users = {w_residual_all_users}")
                ####### test_total
