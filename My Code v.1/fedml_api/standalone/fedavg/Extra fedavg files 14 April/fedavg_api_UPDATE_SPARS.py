...
class FedAvgAPI(object):
   ...
   def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            logging.info("self.p = {}".format(self.p))
            w_locals = []
            vectors_local_AMP = []
            vectors_local_Spar = []
            ...
            for idx, client in enumerate(self.client_list):
                ...
                if self.AMP_TopK[0] == False and self.AMP_TopK[1] == True:
                    ...
                    w_sparsified1 = copy.deepcopy(w_sparsified)
                    vectors_local_Spar.append(w_sparsified1)
                    w_residual_all_users[idx] = copy.deepcopy(w_residual_new) # Renew the residual values
                    #print(w_residual_all_users[idx])
                    ...
                    AMP_all.append(AMP_all_entries)
            ...
            #vectors = []
            #for k in range(len(w_sparsified)):
            #    vectors.append(self.change_weight_vector(w_sparsified[k]))
            length_unique = self.count_non_zero_elements(vectors_local_Spar)
            '''
            with open('length_vector.py', 'w') as f:
                      f.write(f'Count_unique =  {Count_unique.tolist()} \n')
            '''
            ...
            if self.AMP_TopK[0] == True and self.AMP_TopK[1] == False:
               ...
               self._local_test_on_all_clients(round_idx, size_data_TopK_method)
               '''
               ...
               '''
                ...
                #print(f"$$$ vectors_local_Spar = {len(vectors_local_Spar)}")
                size_data_TopK_method = self._TopK_Alg_Size_MPs(vectors_local_Spar, Num_sat_per_orbit) # The size of uploaded parameters in all orbit
                #print(f"$$ size_data_TopK_method = {size_data_TopK_method}")
                ...
                self.model_trainer.set_model_params(w_global)
                if round_idx % self.args.frequency_of_the_test == 0:
                   self._local_test_on_all_clients(round_idx, size_data_TopK_method, length_unique)
   ...
   def count_non_zero_elements(self, AMP_all_entries1):
        #print(f"AMP_all_entries = {AMP_all_entries}")
        #print(f"AMP_all_entries = {len(AMP_all_entries)}")
        #print(AMP_all_entries1[0])
        AMP_all_entries = []
        for k in range(len(AMP_all_entries1)):
            AMP_all_entries.append(self.change_weight_vector(AMP_all_entries1[k]))

        #print(AMP_all_entries)
   ...
   def _size_data_TopK(self,w_sparsification):
            #w_sparsification = [torch.tensor([1,0,1]), torch.tensor([10,0,10]), torch.tensor([1,0,1]), torch.tensor([10,0,10])]
            #print(f"w_sparsification = {w_sparsification[0]}")
            if self.p != 1:
                    x2 = copy.deepcopy(w_sparsification)
                    x1 = []
                    #print(f"##$$$x1 = {x2[0]}")
                    for j in range(len(x2)):
                        all_entries = self.change_weight_vector(x2[j])
                        x1.append(all_entries)
                    #print(f"$$$x1 = {x1[0]}")
                    #print(f"%%%$$$x1 = {x1[0]}")
                    values = []
                    indices = []
                    for k in range(len(x1)):    # This for loop removes the zero elements from each nested element.
                        ...
                        #print(f"x1 = {x1}")
                    #print(f"###values = {indices[0]}")
                    #indices2 = []
                    #for k in range(len(indices1)):
                    #             indices2.append(indices1[k].tolist())
                    '''
                    indices3 = [[] for i in range(len(indices1))]
                    for k1 in range(len(indices1)):
                            for k2 in range(len(indices1[k1])):
                                #print(indices1[k1][k2][0])
                                indices3[k1].append(indices1[k1][k2][0])

                    indices = []
                    for k1 in range(len(indices3)):
                        indices.append(torch.tensor(indices3[k1]))


                    #print(f"HHHHHHH indices = {indices}")


                    indices2 = [[] for k3 in range(len(indices1))]
                    for k1 in range(len(indices1)):
                        for k2 in range(len(indices1[k1])):
                             indices2[k1].append(indices1[k1][k2][0])
                    indices = []
                    for k4 in range(len(indices1)):
                        indices.append(torch.tensor(indices2[k4]))
   ...
   def _local_test_on_all_clients(self, round_idx, size_data_TopK_method, length_unique):

        logging.info("################local_test_on_all_clients : {}".format(round_idx))
        stats_upload_size = {"Upload_Bits_each_Orbit_TopK": size_data_TopK_method}
        wandb.log({"Upload_Bits_each_Orbit_Spar": size_data_TopK_method, "round": round_idx})
        logging.info(stats_upload_size)

        logging.info("################local_test_on_all_clients : {}".format(round_idx))
        stats_upload_len = {"length_unique": length_unique}
        wandb.log({"length_unique": length_unique, "round": round_idx})
        logging.info(stats_upload_len)
