...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                w_sparsification = copy.deepcopy(gradients)
                ...
                w_residual_all_users[idx] = copy.deepcopy(w_residual_new)
                w_locals.append((client.get_sample_number(), copy.deepcopy(w_sparsification1)))



            w_global = self._aggregate_gradients(w_locals, w_global_temp)
    ...
    def _Strom_Threshold_sparsification(self, w_sparsification, w_residual_user, idx):
            ...
            for g in range(len(w_sum_train_residual_main)):
                     ...
                     print(f"Threshold_med = {Threshold_med}")
    ...
    def _Strom_Threshold_sparsification(self, w_sparsification, w_residual_user, idx, p): # Based on the value of maximum
            ...
            for g in range(len(w_sum_train_residual_main)):
                     ...
                     jj_abs = torch.abs(jj)
                     #Threshold_med = torch.median(jj_abs)
                     max_abs = torch.max(torch.abs(jj))
                     Threshold_med = max_abs * (1-p)
                     print(f"Threshold_med = {Threshold_med}")
                     w_sparsification2[list(w_sparsification2.keys())[g]] = torch.where(abs(jj)>Threshold_med, jj, kk)
    ...
    def _aggregate_gradients(self, w_locals, w_global):
        training_num = 0
        ...
        #print(f"averaged_params = {averaged_params['linear.bias'][0]}")
        w_global_new = copy.deepcopy( w_global)
        #print(f"1111111### w_global_new outside = {w_global_new}")
        ...
        for key_idx in range(len( w_global)):
              ...
              values_key_idx2 = list(averaged_params.values())[key_idx]
              values_key_idx3 = values_key_idx1 +  values_key_idx2
