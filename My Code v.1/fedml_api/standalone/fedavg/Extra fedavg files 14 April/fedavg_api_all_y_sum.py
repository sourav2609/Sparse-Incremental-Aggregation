# Implementing FedAvg with gradients, in this code, at first all the received y_i are summed and then we derive the sum of sum x_i.
...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            sum_transform_vectors1 = self.average_transform_AMP_(vectors_local_AMP) # Gets average from \sum_yi

            sum_transform_vectors, error = AMP_Implement_PS(sum_transform_vectors1, N_AMP, n_AMP, k_AMP) # Gets \avg_xi from \avg_yi (AMP)
    ...
    def _aggregate_gradients(self, w_locals, w_global, sum_transform_vectors):
        ...
        k1 = 0
        for i in range(len(list(w_global.keys()))): # This for changes the full local vector to matrix in the size of weights
