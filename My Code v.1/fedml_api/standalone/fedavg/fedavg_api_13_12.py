...
class FedAvgAPI(object):
    def __init__(self, dataset, device, args, model_trainer):
        ...
        self.p = 0.01
        self.AMP_SPAR = [True, False] # % The first entry: AMP and the second entry: SPAR
        self.test_acc_Sync = []
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                if self.AMP_SPAR[0] == False and self.AMP_SPAR[1] == True:

                    w_sparsified, w_residual_new = self._Strom_Threshold_sparsification_all(w_wo_sparsification, w_residual_user, idx, self.p)
                ...
                elif self.AMP_SPAR[0] == True and self.AMP_SPAR[1] == False:
            ...
            if self.AMP_SPAR[0] == True and self.AMP_SPAR[1] == False:
               size_data_SPAR_method = 0
               for idx_orb in range(self.number_orbits):    # Size of all satellites in the constellation in TopK
                   size_data_SPAR_method = size_data_SPAR_method + self._size_data_TopK(vectors_local_Spar[idx_orb*Num_sat_per_orbit: idx_orb*Num_sat_per_orbit + Num_sat_per_orbit]) # Each entry of
               ...
               w_global = self._aggregate_gradients(w_locals, w_global_temp, avg_transform_vectors, self.AMP_SPAR)
               self.model_trainer.set_model_params(w_global)
               if round_idx % self.args.frequency_of_the_test == 0:
                        self._local_test_on_all_clients(round_idx, size_data_AMP_method, size_data_SPAR_method)

            elif self.AMP_SPAR[0] == False and self.AMP_SPAR[1] == True:
    ...
    def change_weight_vector(self, w_sum_train_residual):   # Combine each component of w_sum_train_residual
    ...
    def _size_data_AMP(self, w_sparsification):
        ...
        for i in range(len(flattened_list)):
                   ...
                   #print(f"CHCHCH = {flattened_list[i].element_size() * flattened_list[i].nelement() * 8}")
    ...
    def _size_data_TopK(self,w_sparsification):
            ...
            for j in range(len(group_index)):
                if len(group_index) >= 1:
                    for k in range(1,len(group_index[j])):

                        l3_different1 = [x for x in values[group_index[j][k]] if x not in values[group_index[j][k]]] # Finds the different items of l2 that are not in l1
                        l3 = l1 + l3_different1    # A vector containing all the different and non-different values but once
                        values[group_index[j]] =
            '''
            ...
            for idxx in range(len(group_index)):
                ...
                if len(group_index[idxx]) == 2 and idxx != len(group_index)-1:
                   if group1 != []:
                       ...
                       jj = [x for x in indices[group_index[idxx][0]] if x not in y1_ind[-1]]
                       group1_ind.append([y1_ind[-1], jj])
                       ...
                       jj = [x for x in indices[group_index[idxx][1]] if x not in y2_ind[-1]]
                       group2_ind.append([y2_ind[-1], jj])
                       #group1_ind.append([y1_ind[-1], jj])
                       ...
                       #print(f"%%%%group1_ind = {group1_ind}")
                       group2.append(x0[group_index[idxx][1]])
                       group2_ind.append(indices[group_index[idxx][1]])
                       #print(f"%%%%group2_ind = {group2_ind}")
                    ...
                    y1_ind = copy.deepcopy(group1_ind)
                    group1.append([y1[-1], x0[group_index[idxx][0]]])
                    #jj_same = [x for x in indices[group_index[idxx][0]] if x in y1_ind[-1]]
                    #for kk in range(len(jj_same)):
                    #    aa =
                    #group1.append([y1[-1], tensor(9)])
                    jj = [x for x in indices[group_index[idxx][0]] if x not in y1_ind[-1]]  # Not the same indexes
                    group1_ind.append([y1_ind[-1], jj])
                    ...
                    jj1_ind = [x for x in flatten_group2_ind if x not in flatten_group1_ind]
                    flatten_group1_ind = flatten_group1_ind + jj1_ind
            ...
            #print(f"TopK flattened_list[i].element_size() = {flattened_list_ind_tensor.element_size()}")
            #print(sizee_ind)
    ...
    def _aggregate_gradients(self, w_locals, w_global, avg_transform_vectors, AMP_SPAR):
        ...
        if self.AMP_SPAR[0] == True and self.AMP_SPAR[1] == False:  # Only put the averaged values in avg_transform_vectors in the form of orderdict same as w_global
                    ...
                    if len(w_global[x1].size()) != 1:
                      for j in range(len(w_global[x1])):
                        ...
                        k1 = k1+z
                          ...
                          k1 = k1+z
        elif self.AMP_SPAR[0] == False and self.AMP_SPAR[1] == True:    # Completely derives the average of gradients of weights
                       #print('Hi')
                       training_num = 0
    ...
    def _local_test_on_all_clients(self, round_idx, size_bit, size_data_spar_method):
        ...
        wandb.log({"Test/Acc": test_acc, "comm_round": round_idx})
        wandb.log({"Test/Loss": test_loss, "comm_round": round_idx})
        logging.info(stats)


        stats_upload_size = {"Upload_Bits_each_Orbit_AMP": size_bit}
        wandb.log({"Upload_Bits_each_Orbit_AMP": size_bit, "comm_round": round_idx})
        logging.info(stats_upload_size)


        stats_upload_size = {"Upload_Bits_each_Orbit_Spar": size_data_spar_method}
        wandb.log({"Upload_Bits_each_Orbit_Spar": size_data_spar_method, "comm_round": round_idx})
        logging.info(stats_upload_size)
