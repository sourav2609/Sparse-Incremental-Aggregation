...
### This code is finalized on 25 April. It consists of just Topk
...
class fedsat(object):
    ...
    def train(self):
        ...
        self._local_test_on_all_clients(0, 0, 0,  x_axes = "round")
        for comm_round in range(self.args.comm_round):
            ...
            weight_setup = copy.deepcopy(first_values)
            satellites_weights_in_orbit = []
            for i_sat, client2 in enumerate(self.client_list):
                if i_sat in satellite_idxes_in_orbit:
                       ...
                       if self.approach == 'TopK':
                             w_sparsified, w_residual_new = self._TopK_Threshold_sparsification_all(gradient_sat, w_residual_user, i_sat, self.p) 
                             w_sparsified_flatten = self.change_weight_vector(w_sparsified) 
                             satellites_weights_in_orbit.append(w_sparsified_flatten) 
                             N_AMP = 0
                             n_AMP = 0
                       elif self.approach == 'AMP':      
                              w_sparsified_topk, w_residual_new, w_sparsified, N_AMP, n_AMP, k_AMP, AMP_all_entries = self._AMP_Threshold_sparsification_all(gradient_sat, w_residual_user, i_sat, self.p)
                              #print(f"w_sparsified = {len(w_sparsified)}")
                              satellites_weights_in_orbit.append(w_sparsified)
                               # The size of uploaded parameters in all orbit        
        
                       w_residual_all_users[i_sat]= copy.deepcopy(w_residual_new)
                       w_locals[i_sat] = (client2.get_sample_number(), copy.deepcopy(w_sparsified))
            #print(f"Time required for Train = {Train_Time}")

            if self.approach == 'AMP': 
                 if comm_round <= 10:
                     size_data_AMP_method = self._size_data_AMP(satellites_weights_in_orbit)
                 else: 
                       size_data_AMP_method = 0   
                 #print(f"$$$ size_data_AMP_method = {size_data_AMP_method}")
            elif self.approach == 'TopK':
                  size_data_TopK_method = self._size_data_TopK(satellites_weights_in_orbit)
                  size_data_TopK_method_without_IA = self._size_data_TopK_WO_IA(satellites_weights_in_orbit)
                  #print(f"$$$ size_data_TopK_method = {size_data_TopK_method}")
                  #print(f"$$$ size_data_TopK_method_without_IA = {size_data_TopK_method_without_IA}")


            w_locals_pass_to_aggregate = copy.deepcopy(w_locals) 

            w_global = self._aggregate(w_locals_pass_to_aggregate, w_global_temp, satellite_idxes_in_orbit, N_AMP, n_AMP)
            ...
            if comm_round % self.args.frequency_of_the_test == 0:
               if self.approach == 'AMP':
                   self._local_test_on_all_clients(comm_round+1, size_data_AMP_method, size_data_AMP_method, x_axes = "round")
               elif self.approach == 'TopK':
                   self._local_test_on_all_clients(comm_round+1, size_data_TopK_method, size_data_TopK_method_without_IA, x_axes = "round")                        
    ...
    def tensor_to_weight(self, tensor_values, weight_setup):
                
                k1 = 0
                weight_AMP_dict = copy.deepcopy(weight_setup)
                for i in range(len(list(weight_setup.keys()))): # This for changes the full local vector to matrix in the same shape of weights

                    x1 = list(weight_setup.keys())[i]

                    if len(weight_setup[x1].size()) != 1:
                      for j in range(len(weight_setup[x1])):

                        z = len(weight_setup[x1][j])
                        weight_AMP_dict[x1][j] = torch.tensor(tensor_values[k1:k1+z])
                        k1 = k1+z

                    elif len(weight_setup[x1].size()) == 1: # This if is for the bias part

                          z = len(weight_setup[x1])
                          weight_AMP_dict[x1] = torch.tensor(tensor_values[k1:k1+z])
                          k1 = k1+z  
                return  weight_AMP_dict                 
    
    def _aggregate(self, w_locals, w_global_temp, satellite_idxes_in_orbit, N_AMP, n_AMP):
        ...
        if self.approach == 'AMP':
                ...
                w_global_copy = copy.deepcopy(w_global_temp)
                sum_AMP = torch.zeros(n_AMP)
                for i1 in satellite_idxes_in_orbit:
                        (local_sample_number, local_model_params) = w_locals[i1]
                        w = (local_sample_number) / training_num
                        sum_AMP = sum_AMP + local_model_params * w
                 
                sum_transform_vectors_each_orbit, error_each_orbit = AMP_Implement_PS(sum_AMP, N_AMP, n_AMP)
                print(f"111 ERROR = {error_each_orbit}")
                averaged_params_new = copy.deepcopy(w_global_temp)
                averaged_params_new =  self.tensor_to_weight(sum_transform_vectors_each_orbit, w_global_copy)
    ...
    def _size_data_TopK_WO_IA(self, w_sparsification): # This function derives the size of AMP algorithm for one orbit
    ...
    def _size_data_AMP(self, w_sparsification): # This function derives the size of AMP algorithm for one orbit
                    ...
                    for i in range(levels2):
                        ...
                        for j in range(rows):
                          ...
                          for k in range(columns):
                              ...
                              elif levels1 == 0 and levels2 == 0 and rows == 0:
                                        matrix[k] = lst1[index] 
        #
    ...
    def _AMP_Threshold_sparsification_all(self, w_sparsification, w_residual_user, idx, p): # Based on the value of maximum
            ...
            #print(f"%%% w_sparsified_AMP_y[0] = {w_sparsified_AMP_y[0]}")
            #t2 = time.time()
            #print(f"^^^ t2-t1 = {t2-t1}")
            operator2 = operator.sub
            w_sum_train_residual_main = copy.deepcopy(w_sum_train_residual)
    ...
    def _local_test_on_all_clients(self, idx, size_data, size_data_without_IA, x_axes):
        ...
        wandb.log({"Test/Acc": test_acc, "round": idx, "Size_each_orbit": size_data, "Size_each_orbit_TopK_WO_IA": size_data_without_IA})
        wandb.log({"Test/Loss": test_loss, "round": idx, "Size_each_orbit": size_data,  "Size_each_orbit_TopK_WO_IA": size_data_without_IA})
