...
class fedsat(object):
    ...
    def train(self):
        ...
        for idx1 in range(self.args.client_num_per_round): # Create an initial version of these matrix: w_global_matrix , w_locals
          ...
          w_residual_all_users.append(first_values)   #w_global_matrix has the w_global that each satellite has received fron the GS
          w_local_last.append(first_values)
          w_local_new.append(first_values)
        ...
        for comm_round in range(self.args.comm_round):
            ...
            satellite_idxes_in_orbit = range(num_sat_each_orbit*index_orbit1, num_sat_each_orbit*index_orbit1+num_sat_each_orbit)
            ...
            for i_sat, client2 in enumerate(self.client_list):
                if i_sat in satellite_idxes_in_orbit:
                       ...
                       w_local_new[i_sat] = copy.deepcopy(w)
                       sample_number[i_sat] = copy.copy(client2.get_sample_number())
                       ...
                       for key_idx in range(len(gradient_matrix[i_sat])):   # % Derive the new gradients (g_k(w_k^{n_k+1,I})
                           ...
                           values_key_idx2 = list(w_local_new[i_sat].values())[key_idx]
                       ...
                       ##w_sparsified, w_residual_new = self._TopK_Threshold_sparsification_all(gradient_matrix[i_sat], w_residual_all_users[i_sat], i_sat, self.p)
                       #w_sparsified = copy.deepcopy(gradient_matrix[i_sat])
                       #w_residual_new = copy.deepcopy(w_residual_all_users[i_sat])
                       gradient_matrix_spar[i_sat] = copy.deepcopy(gradient_matrix[i_sat])
                       w_residual_all_users[i_sat] = copy.deepcopy(w_residual_all_users[i_sat])
                       


            #w_global = copy.deepcopy(w_global_withoutchange)
            w_global_pass_to_aggregate = copy.deepcopy(w_global_withoutchange)
            gradient_pass_to_aggregate = copy.deepcopy(gradient_matrix_spar)


            w_global = self._aggregate(first_values, w_global_pass_to_aggregate, gradient_pass_to_aggregate, satellite_idxes_in_orbit, sample_number, comm_round, index_presence_sat_old, index_presence_sat_new)
            index_presence_sat_old = copy.copy(index_presence_sat_new)
    ...
    def _aggregate(self, first_values, w_global, gradient, satellite_idxes_in_orbit, sample_num, comm_round, index_presence_sat_old, index_presence_sat_new):
