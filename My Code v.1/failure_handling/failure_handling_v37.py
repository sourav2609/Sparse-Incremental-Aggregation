...
def Extract_satellite_rise_set(numSat1):
    ...
    duration_visiting = [[] for idxx in range(len(arr_diff_Second_rise))]
    for i1 in range(len(arr_diff_Second_rise)):
            for i2 in range(len(arr_diff_Second_rise[i1])-1):
                    duration_visiting[i1].append(arr_diff_Second_rise[i1][i2+1] - arr_diff_Second_set[i1][i2])
    print(f"$$ duration_visiting = {duration_visiting}")        
...
Iteration_delay_transmission = 1
Iteration_delay_training = 1
...
start_time = np.linspace(10000, 20000, num=10)
#start_time = [0]
#print(start_time)
...
for idx_time_learning in range(Iteration_delay_transmission):
        ...
        for idxx_Sat in range(Num_sat[-1]):
                 #print(index_satellite_delay)
                  delay_learning1[idx_time_learning].extend(np.random.gamma(shape =25, scale = 1, size = 1))
                  #delay_learning1[idx_time_learning].append(0)
                  delay_transmission1[idx_time_learning].extend(np.random.exponential(10, 1))
...
for idx_sat in range(len(Num_sat)):
    ...
    for idx_time_learning in range(Iteration_delay_transmission):
        ...
        for idx_num_start in range(len(start_time)):
            ...
            print(f"idx_without_FH = {idx_without_FH}, time_without_delay = {time_without_delay}, t_now1 = {t_now1}, time_with_FH = {time_with_FH}, time_without_FH = {time_without_FH}, time_updated_multi_hop_FH = {time_updated_multi_hop_FH}")
