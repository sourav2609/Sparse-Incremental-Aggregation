...
from fedml_api.standalone.fedasync_satellite.fedasync_satellite_api import FedAsyncAPI_satellite  #from fedml_api.standalone.fedavg.fedavg_api import FedAvgAPI
...
def add_args(parser):
    ...
    parser.add_argument('--lr', type=float, default=0.7, metavar='LR',
    ...
    parser.add_argument('--alpha_prim', type=int, default=0.3,
                        help='mixing hyper parameter, alpha_prim')
...
def load_data(args, dataset_name):
    ...
    if dataset_name == "mnist":
        ...
        we uniformly sample a fraction of clients each round (as the original FedAvg paper) #FedAvg to be checked (fedasync_satellite?)
        """
...
def create_model(args, model_name, output_dim):
    ...
    if model_name == "lr" and args.dataset == "mnist":
        ...
        model = RNN_OriginalFedAvg() #FedAvg to be checked (FedAsync?)
...
if __name__ == "__main__":
    ...
    parser = add_args(argparse.ArgumentParser(description='fedasync_satellite-standalone')) #parser = add_args(argparse.ArgumentParser(description='FedAvg-standalone')) #FedAvg to be checked (fedasync_satellite?)
    ...
    #wandb.init(project="results_sat", name="fedasync_satellite" + ",alpha_prim: "+str(args.alpha_prim) + " ,lr: " + str(args.lr), config=args)
    #wandb.init(mode="offline", project="results_fed_sat", name="fedasync_satellite" + ",alpha_prim: "+str(args.alpha_prim) + " ,lr: " + str(args.lr)+ " ,Data: " + str(args.dataset), config=args)

    wandb.init(project="LEOsats_schemes", name="fedasync_satellites" + ",-lr = " + str(args.lr)
    + ", Data = " + str(args.dataset)+ " , alpha_prim: "+str(args.alpha_prim) + " , Num_satellites = " +str(args.client_num_per_round),config=args)
    ...
    # In this case, please use our FedML distributed version (./fedml_experiments/distributed_fedavg)   #FedAvg to be checked (fedasync_satellite?)
    initial_weights_matrix = []
    stop = 100
    j_vector = [0, 97, 23, 5, 33, 10, 62, 51, 38, 81]
    #j_vector = [0]
    for i in range(10):

                #j = random.randrange(stop)
                #j_vector.append(j)
                torch.manual_seed(j_vector[i])
                model1 = create_model(args, model_name=args.model, output_dim=dataset[7])
                initial_weights_matrix.append(model1.state_dict())
                #model.reset
    #logging.info(initial_weights_matrix)
    print('initial_weights_matrix = ' + str(initial_weights_matrix))
    print('j_vector = ' + str(j_vector))
    torch.manual_seed(j_vector[0])
    ...
    FedAsyncAPI_satellite = FedAsyncAPI_satellite(dataset, device, args, model_trainer, initial_weights_matrix)
    FedAsyncAPI_satellite.train()
