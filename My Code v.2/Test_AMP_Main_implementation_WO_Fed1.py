...
from import_satellite_visitings_time_v5 import vistings
import numpy as np
import copy
import time
class FEDISL:
#, time_aggregation_ISL
    ...
    def queue(self, TIME_AGGREGATE, queue_operation, time_operation, time_now_vector, vector_time_start, vector_time_end, max_value):
     ...
     if first_operation_queue != 'aggregate':
      for idx_orbit in range(len(vector_time_start)):
       ...
       A = self.dispatcher(first_operation_queue, time_now_vector[idx_orbit], time_operation[idx_orbit], vector_time_start[idx_orbit], vector_time_end[idx_orbit], max_value[idx_orbit],idx_orbit)
      ...
      return time_now_vector
             ...
             time_now_vector = self.dispatcher(first_operation_queue, time_now_vector, time_operation, vector_time_start, vector_time_end, max_value, idx_orbit)
    ...
    def dispatcher(self, first_operation_queue, time_now, time_operation, vector_time_start, vector_time_end, max_value, idx_orbit):
        ...
        if first_operation_queue == 'transmit from GEO to LEO':
        ...
        elif first_operation_queue == 'scatter the weights':
            if time_now != None:
                 time_for_scattering_the_weights = time_operation[1] * (len(vector_time_start) // 2)
                 time_scatter_the_weights = time_now[-1] + time_for_scattering_the_weights + self.time_aggregation_ISL
            ...
            if time_now != None:
                ...
                if A != None:
                    index , time_now = A
                    index_source_sink = [index[idx_orbit][-2], index[idx_orbit][-1]]

                if time_now != None:
                  #print(f"self.num_satellite = {int(self.num_satellite/self.num_planes)}")
                  num_sat_each_orbit = int(self.num_satellite/self.num_planes)
                  #print(f"num_sat_each_orbit//2 = {num_sat_each_orbit//2}")
                  if num_sat_each_orbit%2 == 0:
                     index_compare = abs(index_source_sink[-2] - index_source_sink[-1])
                     #print(f"sou = {index_source_sink[-2]},sin={index_source_sink[-1]},copare={index_compare}")
                     time_for_scattering_the_weights = time_operation[3] * ((len(vector_time_start) // 2) - abs((min(index_compare, num_sat_each_orbit - index_compare))))

                  elif num_sat_each_orbit%2 != 0:
                      index_compare = abs(index_source_sink[-2] - index_source_sink[-1])
                      if index_compare != 0:
                          time_for_scattering_the_weights = time_operation[3] * ((len(vector_time_start) // 2) - abs((min(index_compare, num_sat_each_orbit - index_compare))-1))
                      elif index_compare == 0:
                          time_for_scattering_the_weights = time_operation[3] * (len(vector_time_start) // 2)

                  time_now_after_scattering = time_now[-1] + time_for_scattering_the_weights
                  time_now_after_scattering =  time_now_after_scattering + int(int(self.num_satellite/self.num_planes)/2)*self.time_aggregation_ISL

                  if time_now_after_scattering <= max_value:
                     time_now.append(time_now_after_scattering)
                     return time_now


        elif first_operation_queue == 'transmit from LEO to GEO':
            ...
            if len(vector_time_aggregate) == len(time_now):
                ...
                if   check_all_satellite_weights_to_PS ==  len(time_now):
                        ...
                        time_aggregate = time_aggregate + time_operation[0][5]
...
num_satellite = 40
num_planes = 5
parameter_server = 'NP-4-day_elevation_angle'
inc = 80
time_calculate_the_sink_node = 0.0009
time_aggregation_ISL = 0.0001
...
c_k = 1e3     # Num of cycles per sample
#D =  FL.size_of_data(g=10**-8, BW= 1e6, power=0.5, delta=1e-10, tcomm=0.12)    # Number of data samples of each satellite
D = 3392
Data_bits = 7*8*1e3*1507 # Each data sample is 7kbyte
print(f"D = {Data_bits}")
f_k = 1e9

time_learning = c_k*Data_bits/f_k
...
max_distance_each_satellite = [[0]*8 for idx_orbit in range(len(vector_time_start))]
for idx_orbit in range(5):
     for idx_satellite in range(8):
...
for idx_orbit in range(num_planes):
    min_rate_LEO_LEO[idx_orbit] = FL.Rate(f_c, distance_LEO_LEO[idx_orbit], k, T, B, p_t, g_t, g_r, c)
...
for idx_orbit in range(len(vector_time_start)):
   ...
   time_operations_transmission_from_GEO_to_LEO = FL.end_to_end_transmission_time(max_distance_LEO_GEO[idx_orbit], c, min_rate_LEO_GEO[idx_orbit], D)
   print(f"*****************time_operations_transmission_from_GEO_to_LEO = {time_operations_transmission_from_GEO_to_LEO}")
   time_operations_transmission_from_LEO_to_LEO = FL.end_to_end_transmission_time(distance_LEO_LEO[idx_orbit], c, min_rate_LEO_LEO[idx_orbit], D)
   time_operation_all_orbits.append([time_operations_transmission_from_GEO_to_LEO, time_operations_transmission_from_LEO_to_LEO,
                                     time_learning, time_operations_transmission_from_LEO_to_LEO,
...
while F == True:
       ...
       A = FL.queue(TIME_AGGREGATE, queue_operation, time_operation_all_orbits, time_now, vector_time_start, vector_time_end, max_value)
