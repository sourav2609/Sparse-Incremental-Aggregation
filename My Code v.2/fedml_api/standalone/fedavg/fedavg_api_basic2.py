...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):                        #S Global round
            ...
            for idx, client in enumerate(self.client_list):                     #S for each client run this loop
                ...
                w_locals_gradients.append((client.get_sample_number(), copy.deepcopy(gradients))) #S store the gradients
            ...
            w_global = self._aggregate_gradients(w_locals_gradients, w_global)
    ...
    def _aggregate_gradients(self, w_locals_gradients, w_global):             #S This function implements the aggregation stratagy in the Parameter Server (PS) with gradients
        ...
        for key in range(0, len(averaged_params)):
            w_global[list(w_global.keys())[key]] += averaged_params[list(w_global.keys())[key]]              #S check this very carefully
        return w_global
