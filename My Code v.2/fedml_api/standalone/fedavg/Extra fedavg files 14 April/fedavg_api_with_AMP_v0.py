# Implementing FedAvg with gradients
...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                w_sparsification1, w_residual_new, w_sparsified_transmission, N, n, k, AMP_all_entries = self._AMP_Threshold_sparsification_all(w_wo_sparsification, w_residual_user, idx, p)
            ...
            len_non_zero = self.count_non_zero_elements(self.number_non_zero_entries)   # Calculate the number of non-zero elements in sum_i(y_i).
            print(f"len_non_zero = {len_non_zero}")
            sum_transform_vectors = self.average_transform_AMP_(vectors_local_AMP)

            sum_transform_vectors, error = AMP_Implement_PS(sum_transform_vectors, N, n, k)
            #print(f"&&&&&&&&&&sum_transform_vectors = {len(sum_transform_vectors)}")
            w_global = self._aggregate_gradients(w_locals, w_global_temp, sum_transform_vectors)
    ...
    def _AMP_Threshold_sparsification_all(self, w_sparsification, w_residual_user, idx, p): # Based on the value of maximum
            ...
            for g in range(len(w_sum_train_residual_main)): # for loop to create the sparse vector
                     ...
                     #print(f"jj_abs'= {flatten_matrix}")
                     number_tensor = torch.numel(all_entries)
                     #print(f"$$@@@  number_tensor = {number_tensor}")
                     sort_flatten_matrix = torch.sort(flatten_matrix)
                     #print(f"sort_flatten_matrix = {sort_flatten_matrix}")
                     number_entrries = int(number_tensor * p)
                     #print(f"number_entrries = {number_entrries}")
            ...
            To
            for g in range(len(w_sparsification3)):

               values_tensor = list(w_sparsification3.values())[g].float()
               flatten_values_tensor = torch.flatten(values_tensor)
               all_entries_w_sparsification2.append(flatten_values_tensor)

            AMP_entries_sep_values_cat = torch.cat((all_entries_w_sparsification2[0], all_entries_w_sparsification2[1]))
            AMP_all_entries = torch.flatten(AMP_entries_sep_values_cat)
            #print(f"AMP_entries_sep_values_cat = {AMP_entries_sep_values_cat}")
            #print(f"len_AMP_entries_sep_values_cat = {len(AMP_entries_sep_values_cat)}")
            #print(f"(####^^^^^^^^^^^^ AMP_all_entries = {len(AMP_entries_sep_values_cat)} ")
            with open("values.dat", "w") as f:
                 f.write(str(AMP_entries_sep_values_cat))

            #print(f"(^^^^^^^^^^^^ AMP_all_entries = {AMP_all_entries} ")
            #print(f"$$$$### len(torch.where(AMP_all_entries == 0)) = {len(torch.where(AMP_all_entries != 0)[0])}'")

            w_sparsified_transmission = AMP_Implement_Sat(AMP_all_entries, N, n)
            #print(f"w_sparsified_transmission = {w_sparsified_transmission}")
            #print(f"shape_w_sparsified_transmission = {len(w_sparsified_transmission)}")
            w_residual_new = copy.deepcopy(w_residual_user)
        ...
        for i in range(len(list(w_global.keys()))):
            ...
            if len(w_global[x1].size()) != 1:
              for j in range(len(w_global[x1])):
                ...
                z = len(w_global[x1][j])
                w_local[x1][j] = tensor(sum_transform_vectors[k1:k1+z])
                k1 = k1+z

            elif len(w_local[x1].size()) == 1: # This if is for the bias part

                  z = len(w_local[x1])
                  w_local[x1] = tensor(sum_transform_vectors[k1:k1+z])
                  k1 = k1+z

        #print(f"1111111### w_local = {w_local}")
        #print(f"222222222### averaged_params = {averaged_params}")
        w_global_new = copy.deepcopy(w_global)
        #print(f"$$$w_local$ = {w_local.shape()}")
        for key_idx in range(len(w_global)):
              values_key_idx1 = list(w_global.values())[key_idx]
              values_key_idx2 = list(w_local.values())[key_idx]
              #print(f"len1 = {values_key_idx1}")
              #print(f"len2 = {values_key_idx2}")
              values_key_idx3 = values_key_idx1 +  values_key_idx2
              w_global_new[list(w_global_new.keys())[key_idx]] = values_key_idx3



        #print(f"cccccccccccc w_global_new outside = {w_global_new}")
        return w_global_new
