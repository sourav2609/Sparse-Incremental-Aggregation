...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        round_idx = 0
        self._local_test_on_all_clients(round_idx,0)
        for round_idx in range(1, self.args.comm_round):
            ...
            c = []
            '''
            for i in range(Num_sat_per_orbit): # for AMP_FL
                    a = torch.where(AMP_all_entries[i] != 0.0000000000)
                    b = AMP_all_entries[i][a]
                    c.append([a,b])
                    for i in range(len(c)):
                           size[i] = size + c[i].element_size() * c[i].nelement() * 8
            '''
            #print(f"*****vectors_local_AMP[0] = {vectors_local_AMP[0]}")
            size_bit = 0
            Num_sat_per_orbit = int(len(self.client_list) / self.number_orbits)
            for i in range(Num_sat_per_orbit): # for AMP_FL
                print(f"$$$ self.sum_transform_vectorss[i].element_size() * self.sum_transform_vectorss[i].nelement() * 8 = {vectors_local_AMP[i].element_size() * vectors_local_AMP[i].nelement() * 8}")
                size_bit = size_bit  + (vectors_local_AMP[i].element_size() * vectors_local_AMP[i].nelement() * 8)
                print(f"%%%% {vectors_local_AMP[i].size()}")
                print(f"### size_bit = {size_bit}")
            ...
            if round_idx % self.args.frequency_of_the_test == 0:
                    self._local_test_on_all_clients(round_idx, size_bit)
    ...
    #def upload_size(self, AMP_all_entries):
    ...
    def _local_test_on_all_clients(self, round_idx, size_bit):
        ...
        stats_upload_size = {"Upload_Bits_each_Orbit": size_bit}
        wandb.log({"Upload_Bits_each_Orbit": size_bit, "round": round_idx})
