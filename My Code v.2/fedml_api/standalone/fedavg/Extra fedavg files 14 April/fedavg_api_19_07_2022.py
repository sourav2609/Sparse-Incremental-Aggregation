    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                w_wo_sparsification = copy.deepcopy(gradients)  # Gradients without sparsification
                w_residual_user = copy.deepcopy(w_residual_all_users[idx])
                #print(f"w_residual_user = {w_residual_user}")
                p = 0.1 # How many percent of model parameters are removed
            ...
            size_data_spar_method = self._size_data_spar(vectors_local_Spar)
            size_bit_AMP = self._size_data_AMP(vectors_local_AMP)
            size_bit_without_AMP = 0

            Num_sat_per_orbit = int(len(self.client_list) / self.number_orbits)
            size = [0 for j in range(Num_sat_per_orbit)]

            size_of_upload_bits = 0
            for idx_orb in range(self.number_orbits):
               ...
               #size_of_upload_bits = size_of_upload_bits + (a.element_size() * a.nelement() * 8)
               ...
               size_of_upload_bits = size_of_upload_bits + (self.sum_transform_vectorss[idx_orb].element_size() * self.sum_transform_vectorss[idx_orb].nelement() * 8)
            ...
            if round_idx % self.args.frequency_of_the_test == 0:
                    self._local_test_on_all_clients(round_idx, size_bit_AMP, size_data_spar_method)
    ...
    def _AMP_Threshold_sparsification_all(self, w_sparsification, w_residual_user, idx, p): # Based on the value of maximum
            ...
            for g in range(len(w_sum_train_residual_main)): # for loop to create the sparse vector
                     ...
                     if number_entrries == 0:
                         ...
                         threshold = sort_flatten_matrix[0][number_tensor - number_entrries]
                     w_sparsification2[list(w_sparsification2.keys())[g]] = torch.where(abs(jj)>=threshold, jj.float(), kk) # w_sparsification2 is in the form of w_global in which vector is sparsified.
