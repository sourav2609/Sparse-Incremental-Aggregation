...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):     #Global round
            gradients_tilde_all = [] #To store all the gradients
            ...
            #Global Mask container: which will be decided by single client in round-robin at each iteration
            global_mask = copy.deepcopy(w_global)  #Formatting
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                gradients = copy.deepcopy(w_global_temp)  #Formatting
                ...
                ## Error Feedback: gradients + error
                gradients_tilde = copy.deepcopy(w_global) #Formatting
                ...
                print(f"Max value of gradients for client {idx} is {torch.max(torch.max(torch.abs(self.change_weight_vector(gradients_tilde))))}")
                gradients_tilde_all.append(gradients_tilde)

            #Now a simple procedure to determine: which client decides global mask
            #In this approach: I will check the average of Top-K element of client, the client having the largest av. decides global mask

            av_grad_mag = []
            mask_all = []
            for idx, client in enumerate(self.client_list):
                gradients_idx = gradients_tilde_all[idx]
                gradients_sparse = self.sparsifyMain(gradients_idx, K)
                mask_all.append(self.getMask(gradients_sparse)) #Keep the masks
                flat_gradients_sparsed_sum = torch.sum(torch.abs(self.change_weight_vector(gradients_sparse)))/K #Flatten and get absolute values
                #av_grad_mag.append(torch.min(torch.min(flat_gradients_sparsed_sum[flat_gradients_sparsed_sum > 0])))
                av_grad_mag.append(flat_gradients_sparsed_sum)

            #Obtain the inx of largest value
            master_client_idx = np.argmax(av_grad_mag)
            print(f"Master Client = {master_client_idx}")
            global_mask =  mask_all[master_client_idx]


            ## Now use the global mask obtained from above for all clients
            aggregated_value = copy.deepcopy(w) #Formatting
