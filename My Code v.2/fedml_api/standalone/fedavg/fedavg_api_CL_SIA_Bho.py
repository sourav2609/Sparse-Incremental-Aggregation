    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):     #Global round
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                ##  Sparsification to find the mask for all clients if client_idx matches the counter
                if idx == counter:
                    sparse_gradients = copy.deepcopy(gradients_tilde) #Formatting
                    sparse_gradients = self.sparsifyMain(gradients_tilde, K) #Top-K
                    global_mask = self.getMask(sparse_gradients)
                    #print(f"non-zeros in mask = {self.countNonZeroValues(global_mask)}")

            counter = counter + 1 #Increment the counter, such that it selects next client for mask
            if counter == self.args.client_num_per_round:
                counter = 0
