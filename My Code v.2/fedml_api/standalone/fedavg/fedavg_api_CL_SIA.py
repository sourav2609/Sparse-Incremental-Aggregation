...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                #print(f"aggregated_value = {aggregated_value[list(aggregated_value.keys())[1]].size()}")
                   #print(f"client")                
                print(f"Max value of gradients for client {idx} is {torch.max(torch.max(torch.abs(self.change_weight_vector(gradients))))}")
                ...
                for key in range(len(aggregated_value)):
                     aggregated_value[list(aggregated_value.keys())[key]] = list(gradients.values())[key] + list(aggregated_value.values())[key]
                ## Error Feedback
                ...
                print(f"Non-zeros after spars. = {self.countNonZeroValues(aggregated_value_with_sparsification)}")
    ...
    def Spar_Threshold(self, w_sum_train_residual, all_entries, K): # % Makes the vector sparse
        ...
        #print(f"all indices = {sort_flatten_matrix_index}")
        #print(f"selected indices = {selected_entries_index}")
