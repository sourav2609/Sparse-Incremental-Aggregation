...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        idx = [0]     #Container to store, client indices to aggregate upto and forward in remaining clients
        counter = 0
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            for client_idx in idx: 
                ...
                if client_idx == 0:                                             #Only for the first client
                ...
                w_residual_user = copy.deepcopy(w_residual_all_users[client_idx])      #Obtain the initial residual
                ...
                ## Aggregate for all the clients in idx list
                for key in range(len(aggregated_value)):
                    aggregated_value[list(aggregated_value.keys())[key]] = list(aggregated_value.values())[key] + list(gradients_tilde.values())[key]

                total_sample_num = total_sample_num + client.get_sample_number()


            ##  Then do Sparsification on the aggregate
            aggregated_value_with_sparsification = copy.deepcopy(aggregated_value) #Formatting
            aggregated_value_with_sparsification = self.sparsifyMain(aggregated_value, K)

            ## Update error for the counter'th client as that client only do sparsification
            w_residual_new = copy.deepcopy(w_residual_all_users[counter])
            ...
            w_residual_all_users[counter] = copy.deepcopy(w_residual_new)        # Renew the residual values

            # Update the counter and update idx list
            if counter == self.args.client_num_per_round-1:
                counter = 0
                idx = [0]
            else:    
                counter = counter + 1
            ...
            w_global =  self.parameterServer(w_global, aggregated_value_with_sparsification, total_sample_num) # Update the w_global at the parameter server
