...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            w_global =  self.parameterServer(w_global, aggregated_value, total_sample_num)
            self.model_trainer.set_model_params(w_global)                      
            commCostVec.append(commCost)
            normCostVec.append(data_trans_norm)
            self._local_test_on_all_clients(round_idx, nonZerosOfAggregate, self.args.client_num_per_round, commCost, data_trans_norm)                         
