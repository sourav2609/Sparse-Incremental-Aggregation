...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            gradients_tilde_all_clients = []                                    #For storing all gradients tilde for all clients
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                ## Getting error-compensated Gradients
                for key in range(len(aggregated_value)):
                     gradients_tilde[list(aggregated_value.keys())[key]] = client.get_sample_number()*list(gradients.values())[key] + list(w_residual_user.values())[key]
                gradients_tilde_all_clients.append(gradients_tilde)     #Collect all the gradients
