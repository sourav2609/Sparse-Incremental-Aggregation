...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        round_idx = 0
        print(f"Algo: Aggregate Transfer [proposed]")
        ...
        # Now distributing linear growth in k_i values of Top-k_i for client i
        total_K = K*self.args.client_num_per_round   #Total budget
        k = int((2*total_K)/(self.args.client_num_per_round* (self.args.client_num_per_round+1)))
        #print(f"k value for client = {k}")
        remainder_k = total_K - (k* self.args.client_num_per_round* (self.args.client_num_per_round+1))/2 
        L = 4   # Denotes, remainder_k should be distributed among last L clients in linear growth
        #choosing k_1 for allocating k_1 in linear growth
        k_1 = int((2* remainder_k)/(L*(L+1)))
        remainder_k_1 = remainder_k - (k_1* L* (L+1))/2 # the remaing, I allocate to the last client

        # Now creating a vector of K values
        K_vec = [] 
        for i in range(1, self.args.client_num_per_round-L+1):
            K_vec.append(int(i*k))
        for j in range(self.args.client_num_per_round-L+1, self.args.client_num_per_round+1):
            if (j == self.args.client_num_per_round):
                K_vec.append(int(j*k+ (j - (self.args.client_num_per_round-L))*k_1 + remainder_k_1))
            else:
                K_vec.append(int(j*k+ (j - (self.args.client_num_per_round-L))*k_1))

        print(f"K_vec before reverse = {K_vec} ")
        K_vec.reverse() #Allocating more resources to the first client
        print(f"K_vec after reverse = {K_vec} ")
              
        #K_vec = [K for i in range(self.args.client_num_per_round)]
        #print(f"K_vec new  = {K_vec} ")
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                for key_idx in range(len(gradients)):                           #Derive the gradients
                   ...
                   if idx == 0:
                    #print(f"list(aggregated_value.keys())[key_idx].size() = {aggregated_value[list(aggregated_value.keys())[key_idx]].size()}")
                ...
                for key in range(len(gradients)):
                     gradients_tilde[list(gradients.keys())[key]] = client.get_sample_number()*list(gradients.values())[key] + list(w_residual_user.values())[key]

                ## Transmitting the aggregate or the previous value by Top operation on the same support
                incomingMask = copy.deepcopy(gradients) #Formatting
                if idx == 0:   #For first client, set the mask all 1's
                    for key in range(len(gradients)):
                        incomingMask[list(incomingMask.keys())[key]] = torch.ones(incomingMask[list(incomingMask.keys())[key]].size())
                else:
                    incomingMask = self.getMask(aggregated_value)
                if idx ==0:    
                    mask_client = copy.deepcopy(incomingMask) #Formatting, mask for deciding which values are going out of grad.
                temp = copy.deepcopy(gradients_tilde) #Formatting
                gradients_atIncomingMask = copy.deepcopy(gradients_tilde) #Formatting
                if idx==0:
                    gradients_atIncomingMask = self.Hadamard(gradients_tilde, incomingMask) #Get the values only at the incoming mask
                else:
                    gradients_atIncomingMask = self.Hadamard(gradients_tilde, grad_mask)
                    #To extract the remaing values in aggregated apart from Grad mask
                    grad_mask_complement = self.flipMask(grad_mask)
                    aggregated_remaining_mask = self.Hadamard(incomingMask, grad_mask_complement)
                    aggregated_in_grad_mask = self.Hadamard(aggregated_value, grad_mask)


                for key in range(len(gradients_tilde)):
                    if idx ==0:
                        temp[list(temp.keys())[key]] = torch.where(torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]]) > torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]]+ aggregated_value[list(aggregated_value.keys())[key]]), gradients_atIncomingMask[list(gradients_tilde.keys())[key]], gradients_atIncomingMask[list(gradients_tilde.keys())[key]]+ aggregated_value[list(aggregated_value.keys())[key]])
                        mask_client[list(mask_client.keys())[key]] = torch.where(torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]]) > torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]] + aggregated_value[list(aggregated_value.keys())[key]]), 1, 0)
                    else:    
                        temp[list(temp.keys())[key]] = self.Hadamard(aggregated_value, aggregated_remaining_mask)[list(gradients_tilde.keys())[key]] + torch.where(torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]]) > torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]]+ aggregated_in_grad_mask[list(aggregated_in_grad_mask.keys())[key]]), gradients_atIncomingMask[list(gradients_tilde.keys())[key]], gradients_atIncomingMask[list(gradients_tilde.keys())[key]]+ aggregated_in_grad_mask[list(aggregated_in_grad_mask.keys())[key]])
                        mask_client[list(mask_client.keys())[key]] = torch.where(torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]]) > torch.abs(gradients_atIncomingMask[list(gradients_tilde.keys())[key]] + aggregated_in_grad_mask[list(aggregated_value.keys())[key]]), 1, 0)

                #Sparsify in the same support with lower k
                aggregated_value = self.sparsifyMain(temp, K_vec[idx])

                ## Update error
                sparse_mask = self.getMask(aggregated_value) #This one defines the k_i values that are actually going
                #Find the positions that are outgoing and was selected in mask_client
                if idx ==0:
                    grad_mask = copy.deepcopy(sparse_mask)   #Formatting
                for key in range(len(gradients_tilde)):
                    grad_mask[list(grad_mask.keys())[key]] = torch.where(torch.eq(mask_client[list(mask_client.keys())[key]],sparse_mask[list(mask_client.keys())[key]]), 1, 0)
                grad_mask = self.Hadamard(mask_client, grad_mask)
                gradients_tilde_spars = self.Hadamard(gradients_tilde, grad_mask)
                w_residual_new = copy.deepcopy(w_residual_user)
                for key in range(len(w_residual_user)):
                     w_residual_new[list(w_residual_new.keys())[key]] = list(gradients_tilde.values())[key] - list(gradients_tilde_spars.values())[key]
