...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        # Adaptive K over global rounds
        '''T = self.args.comm_round
        K_ = (K*(T-1))/((T*(T-1))/2)   #First round is  not counted
        K_global = []
        for round_idx in range(1, self.args.comm_round):
            K_global.append(int((T-round_idx)*K_))
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                sparse_gradient = self.sparsifyMain(gradients_tilde, 1*K)
            ...
            print(f"sum of votes greater than > t =  {sum(torch.where(t>7, True, False))}")



            # Global mask, find out the top voted mask
