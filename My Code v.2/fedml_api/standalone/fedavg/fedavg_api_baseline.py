...
class FedAvgAPI(object):
    ...
    def __init__(self, dataset, device, args, model_trainer):
        ...
        self.p = self.args.spar_ratio                                       #args.spar_ratio                        # This denotes the sparsification ratio
    ...
    def train(self):
        ...
        w_residual_last_client = residual   # residual will be kept only for last client
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            print(f"weight vectors = {list(w_global_temp.values())[1].size()}")
            print(f"non-zeros in w_global = {self.countNonZeroValues(w_global)}")
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                print(f"model-new non-zeros = {self.countNonZeroValues(model_new)}")
                ...
                for key_idx in range(len(gradients)):                           #Derive the gradients
                   ...
                   print(f"non-zeros in gradient = {self.countNonZeroValues(gradients)}")   
                   if idx == 0:
                ...
                for key in range(len(gradients)):
                    ...
                    if idx == self.args.client_num_per_round:  #For last client
                        val_idx2 = list(w_residual_last_client.values())[key]
                        val_idx3 = client.get_sample_number()*val_idx1 + val_idx2
                    else:
                        val_idx3 = client.get_sample_number()*val_idx1

                    gradients_tilde[list(gradients_tilde.keys())[key]] = val_idx3

                for key in range(len(aggregated_value_new)):
                    aggregated_value_new[list(aggregated_value_new.keys())[key]] = list(gradients_tilde.values())[key] + list(aggregated_value.values())[key]
                         
                if idx == self.args.client_num_per_round:  #Sparsify for last client
                    gradients_sparsified = self.sparsifyMain(aggregated_value_new, self.p)   #This function implements sparsification
                    for key in range(len(w_residual_last_client)):
                        w_residual_last_client[list(w_residual_last_client.keys())[key]] = list(aggregated_value_new.values())[key] - list(gradients_sparsified.values())[key]            
                    aggregated_value = copy.deepcopy(gradients_sparsified)
                else:
                    aggregated_value = copy.deepcopy(aggregated_value_new)    
                ...
                print(f"data norm = {data_trans_norm}")
    ...
    def sparsifyAndAggregate(self, gradients, w_residual_user, sample_number, aggregated_value):
        ...
        for key in range(len(w_residual_user_new)):
            w_residual_user_new[list(w_residual_user_new.keys())[key]] = list(gradients_tilde.values())[key] - list(gradients_sparsified.values())[key]

        ###Incremental aggregation IA
        aggregated_value_new = copy.deepcopy(aggregated_value) #Formatting
        for key in range(len(aggregated_value_new)):
            aggregated_value_new[list(aggregated_value_new.keys())[key]] = list(gradients_sparsified.values())[key] + list(aggregated_value.values())[key]
    ...
    def Spar_Threshold(self, w_sum_train_residual, all_entries, p): # % Makes the vector sparse
        ...
        number_tensor_accepted = round(number_tensor * p)
