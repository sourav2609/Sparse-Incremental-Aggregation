...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            #print('w_global before = '+str(w_globa l))
            ...
            if round_idx == 1:
                ...
                for key in range(len(w_global_old)):
                    w_global_old[list(w_global_old.keys())[key]] = torch.zeros(list(w_global_old.values())[key].size())  #For the 0th round make old global variable as zero
            ...
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                for key in range(len(aggregated_value_tilde)):
                    val_idx1 = list(aggregated_value.values())[key]
                    #print(f"val = {val_idx1}")
                    val_idx2 = list(w_residual_user.values())[key]
                    val_idx3 = val_idx1 + val_idx2
                    aggregated_value_tilde[list(aggregated_value_tilde.keys())[key]] = val_idx3
                ...
                global_mask = self.getMask(self.sparsifyMain(global_model_diff, Q_G))
  
                ##Local sparsification mask               
                flipped_mask = self.flipMask(global_mask)    #Equivalent to 1 - mask
                Hadamard_temp = self.Hadamard(aggregated_value_tilde, flipped_mask)
                local_mask = self.getMask(self.sparsifyMain(Hadamard_temp, Q_L))

                ##Sparsification
                new_mask = self.addMask(global_mask, local_mask)
                aggregated_value_bar = self.Hadamard(aggregated_value_tilde, new_mask)

                ## Reassign the aggregated value
                for key in range(len(aggregated_value)):
                     aggregated_value[list(aggregated_value.keys())[key]] = aggregated_value_bar[list(aggregated_value_bar.keys())[key]]

                ## Update error
                w_residual_new = copy.deepcopy(w_residual_user)  #Formatting
                for key in range(len(w_residual_new)):
                     w_residual_new[list(w_residual_new.keys())[key]] = list(aggregated_value_tilde.values())[key] - list(aggregated_value_bar.values())[key]
               
                ## Calculation for data transmission
                tmp = self.countNonZeroValues(aggregated_value)
                if idx == 0:   #For the first client
                     norm_val = int(7850*Q_L)*13 + int((Q_G + Q_L)*7850)* 32              

                commCost = commCost + int(7850*Q_L)*13 + int((Q_G + Q_L)*7850)* 32    ## ceil(log2(d)) = 13 and \omega = 32  
                data_trans_norm = data_trans_norm + (int(7850*Q_L)*13 + int((Q_G + Q_L)*7850)* 32 )/norm_val
