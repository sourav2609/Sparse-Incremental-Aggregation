...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        round_idx = 0
        print(f"Algo: Majority Voting Masks")
        ...
        commCostVec = []
        normCostVec = []
        K_global = []  #To store what actual values are going out
        for round_idx in range(1, self.args.comm_round):                        #Global round
            ...
            masks_for_all_clients = copy.deepcopy(w_global)      #Container for storing masks    
            gradients_tilde_all = []  #Container for storing all values of gradients tilde
            sign_mask_all = []
            for idx, client in enumerate(self.client_list):                     #For each client run this loop
                ...
                for key_idx in range(len(gradients)):                           #Derive the gradients
                   ...
                   if idx == 0:
                    ...
                    masks_for_all_clients[list(masks_for_all_clients.keys())[key_idx]] = torch.zeros(masks_for_all_clients[list(masks_for_all_clients.keys())[key_idx]].size())
                ...
                for key in range(len(gradients_tilde)):
                     gradients_tilde[list(gradients_tilde.keys())[key]] = client.get_sample_number()*list(gradients.values())[key] + list(w_residual_user.values())[key]

                # Store gradients tilde
                gradients_tilde_all.append(gradients_tilde)

                ##  Sparsification and obtain the masks to send to the PS
                sparse_gradient = copy.deepcopy(gradients_tilde) #Formatting
                sparse_gradient = self.sparsifyMain(gradients_tilde, 10*K)
                sparse_mask = self.getMask(sparse_gradient)
                temp_grad_tilde = self.Hadamard(gradients_tilde, sparse_mask)
                sign_mask = copy.deepcopy(sparse_mask)  #Formatting
                if idx ==0:
                    sign_aggregate = copy.deepcopy(gradients) #Formatting
                for key in range(len(sign_mask)):
                    if idx ==0:
                        sign_aggregate[list(sign_mask.keys())[key]] = torch.zeros(sign_aggregate[list(sign_mask.keys())[key]].size())
                    sign_mask[list(sign_mask.keys())[key]] = torch.where(temp_grad_tilde[list(sign_mask.keys())[key]] > 0, 1, 0)   #Positive 1, Negative 0
                    sign_aggregate[list(sign_mask.keys())[key]] = sign_aggregate[list(sign_mask.keys())[key]] + sign_mask[list(sign_mask.keys())[key]] 
                sign_mask_all.append(sign_mask)    
                for key in range(len(sparse_mask)):
                    masks_for_all_clients[list(masks_for_all_clients.keys())[key]] = masks_for_all_clients[list(masks_for_all_clients.keys())[key]] + sparse_mask[list(masks_for_all_clients.keys())[key]]


            # After finishing the counting the masks, just take top K values and observe what it is
            t, indices = torch.sort(self.change_weight_vector(masks_for_all_clients)) 
            print(f"sum of votes greater than > 4 =  {sum(torch.where(t>4, True, False))}")

            # Global mask, find out the top voted mask: Agree in position
            global_mask = copy.deepcopy(gradients)  #Formatting
            Votes_more_than_one = copy.deepcopy(gradients) #Formatting
            for key in range(len(masks_for_all_clients)):
                Votes_more_than_one[list(Votes_more_than_one.keys())[key]]= torch.where(masks_for_all_clients[list(masks_for_all_clients.keys())[key]]>1, 1, 0)
            global_mask = self.getMask(self.sparsifyMain(masks_for_all_clients,K))
            #print(f"glo mask before hadar = {global_mask }")
            global_mask = self.Hadamard(global_mask, Votes_more_than_one)  #This step confirms that our mask has votes more than one
            #print(f"glo mask after hadar = {global_mask }")
            masks_for_all_clients = self.Hadamard(masks_for_all_clients, global_mask)
            #print(f"Non-zero positions after = {sum(torch.where(self.change_weight_vector(masks_for_all_clients)>0, True, False))}")
            # We should now think about agreement in signs
            # Finding what signs are in top-K position, whether they match or not
            sign_aggregate = self.Hadamard(sign_aggregate, global_mask)
            sign_threshold = copy.deepcopy(global_mask) #Formatting
            for key in range(len(sign_threshold)):
                print(f"masks_for_all_clients[list(sign_threshold.keys())[key]] = {masks_for_all_clients[list(sign_threshold.keys())[key]]}")
                sign_threshold[list(sign_threshold.keys())[key]] = (masks_for_all_clients[list(sign_threshold.keys())[key]]/2).int()
                print(f"Sign thres = {sign_threshold[list(sign_threshold.keys())[key]]}")
                global_mask[list(sign_threshold.keys())[key]] = torch.where((sign_aggregate[list(sign_threshold.keys())[key]] > sign_threshold[list(sign_threshold.keys())[key]]) | (sign_aggregate[list(sign_threshold.keys())[key]] < masks_for_all_clients[list(sign_threshold.keys())[key]] - sign_threshold[list(sign_threshold.keys())[key]]), 1, 0)
                
            # Budget calculation
            b = 0
            
            b = b + sum(self.change_weight_vector(global_mask))
            K_global.append(b) 
            print(f"K_Global = {K_global}")   
            # Apply the Global mask and update error and aggregate
            for idx, client in enumerate(self.client_list):

                gradients_tilde_idx = gradients_tilde_all[idx]
                sparse_gradient_idx = self.Hadamard(gradients_tilde_idx, global_mask)

                # Update error
                w_residual_new = copy.deepcopy(w_residual_user)
                for key in range(len(w_residual_user)):
                    w_residual_new[list(w_residual_new.keys())[key]] = list(gradients_tilde_idx.values())[key] - list(sparse_gradient_idx.values())[key]
                ...
                for key in range(len(aggregated_value)):
                    aggregated_value[list(aggregated_value.keys())[key]] = list(aggregated_value.values())[key]  + list(sparse_gradient_idx.values())[key]
                
               
            print(f"sum off non-zero positions =  {sum(torch.where(self.change_weight_vector(aggregated_value) != 0, True, False))}")
