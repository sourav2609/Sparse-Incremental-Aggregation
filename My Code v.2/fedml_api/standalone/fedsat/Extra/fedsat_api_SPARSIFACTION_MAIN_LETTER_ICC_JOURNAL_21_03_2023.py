...
class fedsat(object):
    ...
    def train(self):
     ...
     for initiall in range(len(self.initial_weights_matrix)):   # For loop to do the learning with multi initial-weights
        ...
        w_global_matrix_new = []
        w_global_matrix_last = []
        gradient_matrix_new = []
        gradient_matrix_new_Spar = []
        gradient_matrix_last = []
        sample_number = []
        delta_gradient = []
        delta_weight = []
        ...
        for idx1 in range(self.args.client_num_per_round): # Create an initial version of these matrix: w_global_matrix , w_locals
          w_global_matrix_new.append(copy.deepcopy(w_global))   #w_global_matrix has the w_global that each satellite has received fron the GS
          gradient_matrix_new_Spar.append(copy.deepcopy(w_global))   #w_global_matrix has the w_global that each satellite has received fron the GS
          w_global_matrix_last.append(copy.deepcopy(first_values))   #w_global_matrix has the w_global that each satellite has received fron the GS
          gradient_matrix_new.append(copy.deepcopy(first_values))
          gradient_matrix_last.append(copy.deepcopy(first_values))
          delta_gradient.append(copy.deepcopy(first_values))
          delta_weight.append(copy.deepcopy(first_values))
        ...
        for comm_round in range(self.args.comm_round):
            ...
            for i_sat, client2 in enumerate(self.client_list):
                if i_sat in satellite_idxes_in_orbit:
                       ...
                       model_old = copy.deepcopy(w_global_matrix_new[i_sat])
                       gradients_for_new = copy.deepcopy(gradient_matrix_new[i_sat])
                       w_global_temp_to_train = copy.deepcopy(w_global_matrix_new[i_sat])

                       w = client2.train(w_global_temp_to_train)
                       model_new = copy.deepcopy(w)
                       sample_number[i_sat] = copy.deepcopy(client2.get_sample_number())
                       #print(f"000$$$ w_global = {w_global}")

                       for key_idx in range(len(gradients_for_new)):   # % Derive the new gradients (g_k(w_k^{n_k+1,I})
                       ...
                       gradient_matrix_new_Spar[i_sat] = copy.deepcopy(w_sparsified)
                       w_residual_all_users[i_sat] = copy.deepcopy(w_residual_new)
                       #if i_sat == 31:
                       #          print(f"111gradient_matrix_new_Spar[i_sat] == {gradient_matrix_new_Spar[i_sat]}")
                       #print(f"333w_residual_all_users[i_sat] == {w_residual_all_users[i_sat]}")
                       for key_idx1 in range(len(gradients_for_new)):   # % Derive the gradients

                           values_gradient_idx1 = list(gradient_matrix_last[i_sat].values())[key_idx1]
                           values_gradient_idx2 = list(gradient_matrix_new_Spar[i_sat].values())[key_idx1]
                           values_gradient_idx3 = values_gradient_idx2 - values_gradient_idx1
                           delta_gradient[i_sat][list(delta_gradient[i_sat].keys())[key_idx1]] = values_gradient_idx3

                           values_weight_idx1 = list(w_global_matrix_last[i_sat].values())[key_idx1]
                           values_weight_idx2 = list(w_global_matrix_new[i_sat].values())[key_idx1]
                           values_weight_idx3 = values_weight_idx2 - values_weight_idx1
                           delta_weight[i_sat][list(delta_weight[i_sat].keys())[key_idx1]] = values_weight_idx3
                       #if i_sat == 31:
                       #            print(f"222gradient_matrix_last[i_sat] == {gradient_matrix_last[i_sat]}")
                       #            print(f"333delta_gradient[i_sat] == {delta_gradient[i_sat]}")


                       #print(f"222$$$ w_global = {w_global_matrix_new}")
            ...
            delta_gradient_pass_to_aggregate = copy.deepcopy(delta_gradient)
            delta_weight_pass_to_aggregate = copy.deepcopy(delta_weight)

            w_global = self._aggregate(w_global_pass_to_aggregate, delta_gradient_pass_to_aggregate, delta_weight_pass_to_aggregate, satellite_idxes_in_orbit, sample_number, comm_round, index_presence_sat_old, index_presence_sat_new)
            ...
            for j_sat, client1 in enumerate(self.client_list):
                if j_sat in satellite_idxes_in_orbit:
                          gradient_matrix_last[j_sat] = copy.deepcopy(gradient_matrix_new_Spar[j_sat])
                          w_global_matrix_last[j_sat] = copy.deepcopy(w_global_matrix_new[j_sat])
                          w_global_matrix_new[j_sat] = copy.deepcopy(w_global)
    ...
    def _aggregate(self, w_global, delta_gradient, delta_weight, satellite_idxes_in_orbit, sample_num, comm_round, index_presence_sat_old, index_presence_sat_new):
                ...
                for k in averaged_params_new.keys():
                    ...
                    for i1 in satellite_idxes_in_orbit:
                             ...
                             if temp_sum_1 == 1:
                                averaged_params_new[k] = delta_gradient[i1][k] * w
                                averaged_params_new[k] += delta_weight[i1][k] * w
                             else:
                                averaged_params_new[k] += delta_gradient[i1][k] * w
                                averaged_params_new[k] += delta_weight[i1][k] * w
                ...
                for k in new_update_global.keys():
                    #if comm_round == 10000:
                    #    new_update_global[k] = copy.deepcopy(averaged_params_new[k])
