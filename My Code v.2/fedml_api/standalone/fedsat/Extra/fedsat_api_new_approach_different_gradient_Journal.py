...
class fedsat(object):
    ...
    def train(self):
     ...
     for initiall in range(len(self.initial_weights_matrix)):   # For loop to do the learning with multi initial-weights
        ...
        for idx1 in range(self.args.client_num_per_round): # Create an initial version of these matrix: w_global_matrix , w_locals
          ...
          #w_global_matrix.append(copy.deepcopy(first_values))   #w_global_matrix has the w_global that each satellite has received fron the GS
        ...
        for comm_round in range(self.args.comm_round):
            ...
            #print(f"num_sat_each_orbit = {num_sat_each_orbit}")
            ...
            w_locals_without_pass_to_aggregate = copy.deepcopy(w_locals)
            #print(f"%%%%%Before  w_locals_without_pass_to_aggregate = {w_locals_without_pass_to_aggregate}")
            print(f"111w_global = {w_global}")
            for i_sat, client2 in enumerate(self.client_list):
                ...
                if i_sat in satellite_idxes_in_orbit:
                       ...
                       #print(f"w_global_matrix_pass_train = {w_global_matrix_pass_train}")
                       w_global_matrix_pass_without_train = copy.deepcopy(w_global_matrix)

                       w_global_temp = copy.deepcopy(w_global)

                       w = client2.train(w_global_matrix_pass_train)
            ...
            #print(f"^^^^^^After w_locals_without_pass_to_aggregate = {w_locals_pass_to_aggregate}")
            print(f"222w_global = {w_global_temp}")
            #w_global = self._aggregate(w_locals_pass_to_aggregate, w_locals_without_pass_to_aggregate, w_global_temp, satellite_idxes_in_orbit, comm_round,index_presence_sat_old, index_presence_sat_new)
            w_global = self._aggregate(w_locals_pass_to_aggregate, w_global_matrix2, w_global_temp, satellite_idxes_in_orbit, comm_round,index_presence_sat_old, index_presence_sat_new)
            #index_check_orbit_first_visit[index_orbit1] = 1
    ...
    def _aggregate(self, w_locals, w_global_old, w_global_temp, satellite_idxes_in_orbit, comm_round, index_presence_sat_old, index_presence_sat_new):

        ######### This part is for deriving the sum of sample numbers
        #print(satellite_idxes_in_orbit, index_presence_sat_new)
        #print(f"--- comm_round = {comm_round}")
        #print(f"--- satellite_idxes_in_orbit = {satellite_idxes_in_orbit}")
        #print(f"111  w_locals = {w_locals}")
        #print(f"222  w_global_old = {w_global_old}")
        training_num = 0
        for idx2 in range(len(w_locals)):
            (sample_num, averaged_params) = w_locals[idx2]
            if sample_num != 0:
        ...
        #for idx in satellite_idxes_in_orbit[0]:
        ...
        #print(f"AAA###averaged_params_new = {averaged_params_new}")
        #for idx in satellite_idxes_in_orbit:
        averaged_params_old = w_global_old[satellite_idxes_in_orbit[0]]

        for k in averaged_params_old.keys():
            temp_sum_2 = 0
            for i2 in satellite_idxes_in_orbit:

                     local_model_params = w_global_old[i2]
                     temp_sum_2 += 1
                     w = (local_sample_number) / training_num
                     #print(f"$$$ w = {w}")
                     #w = 1/40
                     if temp_sum_2 == 1:
                        averaged_params_old[k] = local_model_params[k] * w
                     else:
                        averaged_params_old[k] += local_model_params[k] * w

        #print(f"BBBAAA###averaged_params_old = {averaged_params_old}")
        ...
        if comm_round == 0:
                   ...
                   w_global_temp_new = copy.deepcopy(first_values)
                   #print(f"CCCCCCCC w_global_temp = {w_global_temp}")
                   #print(f"DDD new_weight_multiply = {new_weight_multiply}")
                   for k in w_global_temp_new.keys():
                                #new_weight_multiply* new_weight_multiply*
                                w_global_temp_new[k] = (new_weight_multiply*w_global_temp[k]) - averaged_params_old[k] + averaged_params_new[k]



        #print(f"333  w_global_temp_new = {w_global_temp_new}")
