...
class fedsat(object):
    ...
    def train(self):
     ...
     for initiall in range(len(self.initial_weights_matrix)):   # For loop to do the learning with multi initial-weights
        ...
        #for i_sat3, client3 in enumerate(self.client_list):
           #sample_num.append(client3.get_sample_number())
        ...
        for comm_round in range(self.args.comm_round):
            ...
            #index_presence_sat_new[index_orbit1] = self.num_orbit
            #print(f"111 index_presence_sat_new = {index_presence_sat_new}")
            ...
            for i_sat, client2 in enumerate(self.client_list):
                ...
                if i_sat in satellite_idxes_in_orbit:
                       ...
                       #print(f"i_sat = {i_sat}")
                       #if i_sat == 31:
                       #       print(f"111 w_global_matrix[i_sat] = {w_global_matrix[i_sat]}")
                       ...
                       '''
                       if i_sat == 31:
                           #print(f"000w = {w['linear.weight'][0][12]}")
                           print(f"111w_global_matrix_pass_sparsification = {w_global_matrix_pass_sparsification['linear.weight'][0][12]}")
                           print(f"222w_trained_copy = {w_trained_copy['linear.weight'][0][12]}")
                       '''
                       ...
                       '''
                       if i_sat == 31:
                            print(f"333 gradient_matrix_new = {gradient_matrix_new[i_sat]['linear.weight'][0][12]}")
                       '''
            ...
            for j_sat, client1 in enumerate(self.client_list):
                if j_sat in satellite_idxes_in_orbit:
                          ...
                          '''
                          if j_sat == 31:
                           print(f"444 w_global_matrix[j_sat] = {w_global_matrix[j_sat]['linear.weight'][0][12]}")
                          '''
